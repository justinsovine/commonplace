...
export default function BookingsList({ bookings }: { bookings: Booking[] }) {
    ...
    const getBadgeColor = (status: string) => {
        switch (status) {
            ...
            case 'pending':
                return 'bg-yellow-600 text-stroke';
            case 'cancelled':
                    ...
                    <div className='flex flex-col md:flex-row gap-4'>
                        ...
                        <div className='flex-1'>
                            <div className='flex justify-between items-start'>
                                ...
                                <span
                                    className={`px-3 py-1 text-xs rounded-full font-medium ${getBadgeColor(booking.status)}`}
                                >
                                    {booking.status}
                                </span>
                            </div>
                            ...
                            <div className='flex gap-3 mt-2'>
                                ...
                                {booking.status !== 'cancelled' && (
                                    <button className='text-sm px-3 py-1 rounded bg-red-600 text-white hover:bg-red-700 transition-colors'>
                                        Cancel
                                    </button>
                                )}
                            </div>
